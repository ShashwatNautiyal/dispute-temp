...
export const TransactionsModalHeader: Component<{
  ...
  <div class="flex-shrink-0 relative bg-[#F2F2F2] h-[234px]">
    <div class="absolute bottom-6 left-3 flex flex-col gap-2">
      <h2 class="text-[#1D1D1F] font-semibold text-[24px] leading-9">
        {props.name}
      </h2>
      <p class="text-[#494949] font-normal text-[15px] leading-6">
        {props.description ?? `Add and manage your ${props.name} accounts.`}
      </p>
    </div>
...
export const TransactionsModalContent: Component<{
  ...
  const [selectedProvider, setSelectedProvider] = createSignal<
    "All" | (typeof PROVIDERS)[number]["name"]
  >("All");
    ...
    <>
      <div class="flex-shrink-0 relative bg-[#F2F2F2] h-[234px]">
        <div class="absolute bottom-6 left-3 flex flex-col gap-2">
          ...
          <p class="text-[#494949] font-normal text-[15px] leading-6">
            Choose, Connect and Protect your business.
          </p>
        </div>
      </div>
  ...
  const PaymentProcessor: Component<{
    ...
    const [showAddAccountToProcessor, setShowAccountToProcessor] =
      createSignal(false);
    const [userHasAccountsOnFirstRender, setUserHasAccountsOnFirstRender] =
      createSignal((user.accountsOnProvider?.[props.name] ?? []).length > 0);
    createEffect(
      on(
        () => props.name,
        (provider_name) => {
          setUserHasAccountsOnFirstRender(
            (user.accountsOnProvider?.[provider_name] ?? []).length > 0
          );
        }
      )
    );
    ...
    const AddAccountToProcessor: Component = () => {
      ...
      const [currentStep, setCurrentStep] = createSignal<Steps>(
      ...
      const InputContainer: Component<{
        ...
        <div class="w-full py-3 px-2">
          <input
            required
            type={props.type}
            class="bg-transparent w-full outline-none placeholder:text-[#1D1D1F99] text-[#1D1D1F] text-[15px] leading-[18px] font-normal"
      ...
      const addAccountFromState = () =>
        batch(() => {
          ...
          batch(() => {
            setUser("accountsOnProvider", props.name, (prev) => [
              ...prev,
              {
                flowCount: 2,
            ...
            <div class="flex flex-col gap-2 w-[356px]">
              <div class="flex items-center gap-2">
                ...
                <Show when={currentStep() !== Steps.CONTACT_DETAILS}>
                  <button
                    type="button"
                    onClick={() => setCurrentStep(Steps.CONTACT_DETAILS)}
                  >
                    <EditIcon />
                  </button>
                </Show>
              ...
              <div class="flex flex-col border border-[#1D1D1F1F] divide-y divide-[#1D1D1F1F] rounded-lg">
                <Show
                  when={currentStep() === Steps.CONTACT_DETAILS}
                  fallback={
                    <StepRecapContainer
                      value={Object.values(state.contact).join(", ")}
                    />
                  }
                >
                  <InputContainer
                    ...
                    onValueChange={(value) =>
                      setState("contact", "fullName", value)
                    }
                  />
                  <InputContainer
                    type="tel"
                  ...
                  <InputContainer
                    ...
                    onValueChange={(value) =>
                      setState("contact", "address", value)
                    }
                  />
                </Show>
              </div>
              ...
              <div class="flex flex-col gap-2 w-[356px]">
                <div class="flex items-center gap-2">
                  ...
                  <Show when={currentStep() !== Steps.BUSINESS_DETAILS}>
                    <button
                      type="button"
                      onClick={() => setCurrentStep(Steps.BUSINESS_DETAILS)}
                    >
                      <EditIcon />
                    </button>
                  </Show>
                ...
                <div class="flex flex-col border border-[#1D1D1F1F] divide-y divide-[#1D1D1F1F] rounded-lg">
                  <Show
                    when={currentStep() === Steps.BUSINESS_DETAILS}
                    fallback={
                      <StepRecapContainer
                        value={Object.values(state.business).join(", ")}
                      />
                    }
                  >
                    <InputContainer
                    ...
                    <InputContainer
                      ...
                      onValueChange={(value) =>
                        setState("business", "supportEmail", value)
                      }
                    />
                    <InputContainer
                      type="url"
                      ...
                      onValueChange={(value) =>
                        setState("business", "websiteUrl", value)
                      }
                    />
                  </Show>
                </div>
    ...
    return (
      <>
        <TransactionsModalHeader
          name={props.name}
          showAddButton={
            userHasAccountsOnFirstRender() && !showAddAccountToProcessor()
          }
        ...
        <Show
          when={userHasAccountsOnFirstRender()}
          fallback={
            <div
              class="p-3 pt-10 flex flex-col gap-8 items-center h-full overflow-auto"
        ...
        >
          <Show
            when={!showAddAccountToProcessor()}
            fallback={
              <div
                class="p-3 pt-10 flex flex-col gap-8 items-center h-full overflow-auto"
                style={{ "scrollbar-gutter": "stable" }}
          ...
          >
            <div
              class="p-2 overflow-hidden hover:overflow-y-scroll flex flex-col gap-2"
              style={{ "scrollbar-gutter": "stable" }}
            >
  ...
  return (
    <div class="flex divide-x h-full">
      ...
      <div class="flex flex-col w-full">
        <Show
          when={selectedProvider() !== "All"}
          fallback={<PaymentProcessorAll />}
        >
          <PaymentProcessor
            name={
              selectedProvider() as Exclude<
                ReturnType<typeof selectedProvider>,
                "All"
              >
            }
          />
  ...
  return (
    <Modal open={showModal()} onClose={close}>
      <div class="relative bg-white w-[750px] h-[600px] rounded-[16px] overflow-hidden">
        <button class="absolute top-2 right-2" onClick={close}>
          Close
        </button>
